    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
          ...
          px-3 py-2 rounded-pill mb-3">
                        {{ $status === 'aktif' ? 'Konfirmasi Penyelesaian' : 'Aktifkan Kembali
                        Kegiatan' }}
                    </span>
                    <h3 class="fw-bold mb-2">
                        @if($status === 'aktif') Selesaikan Kegiatan? @else Aktifkan Kembali?
                        @endif
                    </h3>
                    <p class="text-muted mb-0 lh-lg px-lg-4">
                        @if($status === 'aktif') Kegiatan
                        <strong>
                            {{ $namaKegiatan }}
                        </strong>
                        akan ditutup dan operasional presensi maupun infaq tidak dapat diubah
                        kembali. @else Kegiatan
                        <strong>
                            {{ $namaKegiatan }}
                        </strong>
                        akan dibuka kembali sehingga presensi dan infaq dapat dilanjutkan. @endif
                    </p>
                </div>
                {{-- WARNING --}}
                    ...
                    <div class="d-flex align-items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-muted mb-0 fs-13">
                                @if($status === 'aktif') Pastikan seluruh presensi dan infaq kegiatan
                                telah diinput sebelum kegiatan diselesaikan. @else Pengaktifan kembali
                                kegiatan akan membuka akses operasional presensi dan infaq. @endif
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer border-0 pt-0 px-4 pb-4 justify-content-center">
                ...
                <button type="button"
                    class="btn {{ $status === 'aktif' ? 'btn-success' : 'btn-primary' }} rounded-pill px-4"
